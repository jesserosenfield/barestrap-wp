<?php
	if(!is_admin()) {
		...
		*/				
		...
		function bf_auto_excerpt_more( $more ) {
			return '';
		}
		add_filter( 'excerpt_more', 'bf_auto_excerpt_more', 20 );
		
		function bf_custom_excerpt_more( $output ) {
			return preg_replace('/<a[^>]+>Continue reading.*?<\/a>/i','',$output);
		}
		add_filter( 'get_the_excerpt', 'bf_custom_excerpt_more', 20 );
